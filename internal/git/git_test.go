...
func TestPushRefspecFastForward(t *testing.T) {
	remoteDir := t.TempDir()
	cmd := exec.Command("git", "init", "--bare")
	cmd.Dir = remoteDir
	if err := cmd.Run(); err != nil {
	...
	// Push main and a merge-target both pointing at the initial commit.
	for _, b := range []string{mainBranch, "merge-target"} {
		cmd = exec.Command("git", "push", "origin", "HEAD:refs/heads/"+b)
		cmd.Dir = localDir
		if err := cmd.Run(); err != nil {
	...
	// Advance main by one commit and push it.
	newSHA := commitFile(t, localDir, "hotfix.txt", "fix", "hotfix on main")
	cmd = exec.Command("git", "push", "origin", "HEAD:refs/heads/"+mainBranch)
	cmd.Dir = localDir
	if err := cmd.Run(); err != nil {
		t.Fatalf("push main: %v", err)
	}

	// Fast-forward merge-target onto main's new tip via refspec push.
	refspec := fmt.Sprintf("%s:refs/heads/merge-target", newSHA)
	if err := g.PushRefspec("origin", refspec, false); err != nil {
		t.Fatalf("PushRefspec ff: %v", err)
	}
	...
	if tip != newSHA {
		t.Errorf("merge-target tip = %s, want %s", tip, newSHA)
	}
}

func TestPushRefspecNonFastForwardRejected(t *testing.T) {
	remoteDir := t.TempDir()
	cmd := exec.Command("git", "init", "--bare")
	cmd.Dir = remoteDir
	if err := cmd.Run(); err != nil {
	...
	// merge-target diverges: it gets a commit that never lands on main.
	if err := g.CreateBranch("merge-target"); err != nil {
		t.Fatalf("CreateBranch: %v", err)
	}
	if err := g.Checkout("merge-target"); err != nil {
		t.Fatalf("Checkout: %v", err)
	}
	commitFile(t, localDir, "divergent.txt", "only here", "divergent commit")
	if err := g.Checkout(mainBranch); err != nil {
		t.Fatalf("Checkout main: %v", err)
	}
	// main advances separately.
	mainSHA := commitFile(t, localDir, "main.txt", "main work", "main commit")

	for _, b := range []string{mainBranch, "merge-target"} {
		cmd = exec.Command("git", "push", "origin", b)
		cmd.Dir = localDir
		if err := cmd.Run(); err != nil {
	...
	// A non-force push of main onto the divergent merge-target must be rejected.
	refspec := fmt.Sprintf("%s:refs/heads/merge-target", mainSHA)
	if err := g.PushRefspec("origin", refspec, false); err == nil {
		t.Fatal("PushRefspec non-ff: expected rejection, got nil")
	}

	// Sanity: base is an ancestor of main but merge-target is not (real divergence).
	mtIsAncestor, err := g.IsAncestor("origin/merge-target", "origin/"+mainBranch)
	if err != nil {
		t.Fatalf("IsAncestor: %v", err)
	}
	if mtIsAncestor {
		t.Error("expected merge-target NOT to be ancestor of main (divergence)")
	}
	if _, err := g.IsAncestor(base, "origin/"+mainBranch); err != nil {
		t.Fatalf("IsAncestor base: %v", err)
	}
}
